...
fn parse_search(mut teams: Vec<Team>) -> SearchResult {
    if teams.len() == 0 {
        SearchResult::None
    } else if teams.len() == 1 {
        SearchResult::Team(teams.pop().unwrap())
    } else {
...
impl Team {
    pub fn find(tournament_id: &str, name: &str) -> reqwest::Result<SearchResult> {
        let url = format!("{}tournaments/{}/teams?name={}", CLOUDFRONT, tournament_id, name);
        let teams: Vec<Team> = reqwest::blocking::get(&url)?.json()?;
        
        Ok(parse_search(teams))
    }
...
impl Match {
    pub fn find(stage_id: &str, team_id: &str, round: u8) -> reqwest::Result<Option<Match>> {
...
mod tests {
    ...
    fn load_matches() -> Result<Vec<Match>, Box<dyn Error>> {
        Ok(serde_json::from_reader::<BufReader<File>, Vec<Match>>(
            BufReader::new(File::open("./test_data/matches.json")?)
        )?)
    }

    fn load_teams(filename: &str) -> Result<Vec<Team>, Box<dyn Error>> {
        Ok(serde_json::from_reader::<BufReader<File>, Vec<Team>>(
    ...
    fn parse_extended_match() -> Result<(), Box<dyn Error>> {
        let _ = serde_json::from_reader::<BufReader<File>, Vec<Match>>(
            BufReader::new(File::open("./test_data/match.json")?)
        )?;
        Ok(())
    }
    ...
    #[test]
    fn test_parse_search() -> Result<(), Box<dyn Error>> {
        let team = load_teams("./test_data/team_one.json")?;
        match parse_search(team) {
            SearchResult::Team(_) => Ok(()),
            SearchResult::Teams(_) => panic!("more than one team returned"),
            SearchResult::None => panic!("nothing found"),
        }
    }

    #[test]
    fn parse_search_multiple_results() -> Result<(), Box<dyn Error>> {
        let teams = load_teams("./test_data/teams.json")?;
        match parse_search(teams) {
            SearchResult::Team(_) => panic!("only found 1 team"),
            SearchResult::Teams(_) => Ok(()),
            SearchResult::None => panic!("no team found"),
        }
    }
}
